...
for (const [index, email] of sequence.emails.entries()) {
  const content = `<!--[if mso]><table role="presentation" align="center" width="600" cellpadding="0" cellspacing="0"><tr><td><![endif]--><table role="presentation" width="100%" cellpadding="0" cellspacing="0" align="center" style="border-collapse:collapse;max-width:600px;margin:auto;background:#f6f2e9;color:#43382d">
...
<tr><td><img src="https://www.louiemae.com/images/prelaunch/${email.image.replace('.webp', '-email.jpg')}" alt="${index === 0 ? 'Staggered ceramic vases in a sunlit room' : index === 1 ? 'An open book and olive branch on linen' : index === 2 ? 'An inviting living room with a rustic wood table' : 'A cream dress in a sunlit meadow'}" width="600" style="display:block;width:100%;height:auto;border:0"></td></tr>
...
[DRAFT — business mailing address pending]<br>{% unsubscribe 'Unsubscribe' %}</td></tr></table><!--[if mso]></td></tr></table><![endif]-->`;
  ...
  previews.push(`<section><header><strong>DRAFT ${index + 1} · DAY ${email.day}</strong><h2>${escape(email.subject)}</h2><p>${escape(email.preview)}</p></header>${content.replace('[DRAFT — business mailing address pending]', '[Business mailing address — supplied in Klaviyo]').replace("{% unsubscribe 'Unsubscribe' %}", 'Unsubscribe')}</section>`);
}
fs.writeFileSync(path.join(dir, 'waitlist-review.html'), `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Louie Mae — Welcome Email Drafts</title><style>body{margin:0;background:#eae4d9;color:#43382d}main{max-width:680px;margin:auto;padding:32px 12px}header{font-family:Arial,sans-serif;padding:24px}section{margin-bottom:64px}h2{font-size:22px}aside{font-family:Arial,sans-serif;line-height:1.6;padding:24px;background:#fff9ed}</style></head><body><main><aside><strong>Louie Mae · Welcome sequence for review</strong><p>Drafts only. No emails are active or scheduled. Sender: Louie Mae &lt;withlove@louiemae.com&gt;. Days are measured from joining the list. Reply-to: hello@louiemae.com. A business mailing address must be approved before sending. The unsubscribe link is supplied by Klaviyo. Test delivery and owner review are required before activation.</p></aside>${previews.join('')}</main></body></html>`);
